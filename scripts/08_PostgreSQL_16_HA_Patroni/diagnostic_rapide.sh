...
for NODE in "db-master-01:$DB_MASTER_IP" "db-slave-01:$DB_SLAVE1_IP" "db-slave-02:$DB_SLAVE2_IP"; do
    ...
    ROLE=$(curl -s -m 5 -u "${PATRONI_USER}:${PATRONI_PASS}" "http://${IP}:8008/" 2>/dev/null | grep -o '"role":"[^"]*"' | cut -d'"' -f4 || echo "unknown")
    STATE=$(curl -s -m 5 -u "${PATRONI_USER}:${PATRONI_PASS}" "http://${IP}:8008/" 2>/dev/null | grep -o '"state":"[^"]*"' | cut -d'"' -f4 || echo "unknown")
...
for NODE in "haproxy-01:$HAPROXY1_IP" "haproxy-02:$HAPROXY2_IP"; do
    ...
    ((TOTAL++))
    echo -n "    Stats (8404): "
    if curl -sf -m 5 "http://${IP}:8404/" 2>&1 | head -20 | grep -qi "haproxy\|statistics\|DOCTYPE"; then
        echo -e "$OK"
        ((SUCCESS++))
    else
        echo -e "$KO"
