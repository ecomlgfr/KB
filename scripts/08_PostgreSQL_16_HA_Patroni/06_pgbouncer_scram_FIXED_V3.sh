...
EOF
    ...
    # Démarrer PgBouncer avec l'image CORRECTE
    echo "  → Démarrage PgBouncer..."
    
    # CORRECTION V3: Utiliser l'image officielle edoburu/pgbouncer
    # ou construire notre propre image si nécessaire
    
    # Option 1: Essayer l'image edoburu (plus maintenue et populaire)
    PGBOUNCER_IMAGE="edoburu/pgbouncer:1.21.0-p0"
    
    # Vérifier si l'image existe localement, sinon la pull
    if ! docker images | grep -q "edoburu/pgbouncer"; then
        echo "  → Pull de l'image PgBouncer ($PGBOUNCER_IMAGE)..."
    ...
    if ss -tln | grep -q ":6432 "; then
        ...
        ss -tln | grep -E ":(5432|6432|8008)" || echo "    Aucun port PostgreSQL détecté"
        exit 1
    fi
...
if timeout 10 bash -c "PGPASSWORD='$POSTGRES_PASSWORD' psql -h $HAPROXY1_IP -p 6432 -U postgres -d postgres -c 'SELECT 1' >/dev/null 2>&1"; then
    ...
    timeout 5 bash -c "PGPASSWORD='$POSTGRES_PASSWORD' psql -h $DB_MASTER_IP -p 5432 -U postgres -d postgres -c \"SELECT 'Direct OK'\"" 2>&1 | head -3
    echo ""
    echo "  → Debug: Logs PgBouncer sur haproxy-01"
    ssh -o StrictHostKeyChecking=no root@"$HAPROXY1_IP" "docker logs pgbouncer --tail 20" 2>&1 | grep -i "error\|fatal\|scram\|listening" || true
fi
...
if timeout 10 bash -c "PGPASSWORD='$POSTGRES_PASSWORD' psql -h $HAPROXY1_IP -p 6432 -U postgres -d pgbouncer -c 'SHOW POOLS;' 2>/dev/null"; then
    :
else
...
echo ""
echo "═══════════════════════════════════════════════════════════════════"
echo -e "$OK Installation PgBouncer terminée (V3 - Image corrigée)"
echo "═══════════════════════════════════════════════════════════════════"
echo ""
echo "🐳 Image Docker utilisée:"
echo "   • edoburu/pgbouncer:1.21.0-p0 (ou bitnami/pgbouncer:latest en fallback)"
echo ""
