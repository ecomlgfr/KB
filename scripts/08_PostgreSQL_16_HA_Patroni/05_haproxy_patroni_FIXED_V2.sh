...
for PROXY_NODE in "haproxy-01:$HAPROXY1_IP" "haproxy-02:$HAPROXY2_IP"; do
    ...
    # CORRECTION : Ne pas mettre db-master dans le backend replicas !
...
# CORRECTION : Ne mettre QUE les replicas ici !
...
if curl -sf "http://${HAPROXY1_IP}:8404/" | grep -q "Statistics" 2>/dev/null; then
    ...
    echo -e "$WARN (peut prendre quelques secondes)"
fi

echo ""
echo "═══════════════════════════════════════════════════════════════════"
echo -e "$OK Installation HAProxy terminée (V2 - Corrigée)"
echo "═══════════════════════════════════════════════════════════════════"
echo ""
echo "📊 Vérification HAProxy Stats:"
echo "   curl http://${HAPROXY1_IP}:8404/ | grep -E 'db-master|db-slave'"
echo ""
echo "   Attendu:"
echo "   • Backend postgres_master: db-master-01 UP, slaves en backup"
echo "   • Backend postgres_replicas: SEULEMENT db-slave-01 et db-slave-02"
echo ""
echo "📋 Prochaine étape: PgBouncer avec SCRAM-SHA-256"
echo "   bash 06_pgbouncer_scram_FIXED_V2.sh"
echo ""
