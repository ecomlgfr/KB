...
# Créer le répertoire credentials si absent
mkdir -p "$CREDS_DIR"
...
# Vérifier et charger ou générer les credentials
...
if [ -f "$CREDS_DIR/postgres.env" ]; then
    ...
    if [ -z "${POSTGRES_PASSWORD:-}" ] || [ -z "${REPLICATOR_PASSWORD:-}" ] || [ -z "${PATRONI_API_PASSWORD:-}" ]; then
        echo "  Credentials incomplets, régénération nécessaire..."
        NEED_GENERATION=true
    else
        echo "  Credentials existants conservés:"
        echo "    PostgreSQL: ${POSTGRES_PASSWORD:0:4}****${POSTGRES_PASSWORD: -4}"
        echo "    API Patroni: ${PATRONI_API_PASSWORD:0:4}****${PATRONI_API_PASSWORD: -4}"
    fi
else
...
# URLs de connexion pour les applications
...
EOF
    ...
    echo "  Nouveaux credentials générés et sauvegardés:"
    echo "    PostgreSQL: $POSTGRES_PASSWORD"
    echo "    Replicator: $REPLICATOR_PASSWORD"
    echo "    API Patroni: $PATRONI_API_PASSWORD"
    echo ""
    echo "  Fichier: $CREDS_DIR/postgres.env"
fi
...
echo ""
echo "2. Préparation des nœuds..."
echo ""

for ip in 10.0.0.120 10.0.0.121 10.0.0.122; do
    hostname=$(awk -F'\t' -v ip="$ip" '$3==ip {print $2}' "$SERVERS_TSV")
    echo -n "  $hostname ($ip): "
    
    # Vérifier si c'est un leader actif
    IS_LEADER=false
    if [ "$FORCE_WIPE" = false ]; then
...
# Générer patroni.yml avec les variables
...
ssh root@10.0.0.120 bash <<'START'
docker run -d \
  --name patroni \
...
for server in "10.0.0.121:db-slave-01" "10.0.0.122:db-slave-02"; do
    ...
    ssh root@"$ip" bash -s "$hostname" <<'START'
HOSTNAME="$1"
docker run -d \
  --name patroni \
  --hostname $HOSTNAME \
...
echo ""
echo "5. Vérification et création des bases..."
echo ""
...
if [ -n "$LEADER_IP" ]; then
    ssh root@"$LEADER_IP" bash -s "$POSTGRES_PASSWORD" <<'SETUP_DBS'
PG_PASSWORD="$1"
...
echo "═══════════════════════════════════════════════════════════════════"
echo ""
echo "Credentials sauvegardés dans: $CREDS_DIR/postgres.env"
echo ""
echo "Test de connexion:"
echo "  export PGPASSWORD='$POSTGRES_PASSWORD'"
echo "  psql -h 10.0.0.10 -p 5432 -U postgres -d keybuzz"
echo ""
echo "API Patroni:"
echo "  curl -u patroni:$PATRONI_API_PASSWORD http://${LEADER_IP:-10.0.0.120}:8008/cluster"
echo ""
...
# KeyBuzz HBA overlay (idempotent)
IP_DB1=$(awk -F"	" '$2=="db-master-01"{print $3}' "$SERVERS_TSV")
IP_DB2=$(awk -F"	" '$2=="db-slave-01"{print $3}' "$SERVERS_TSV")
IP_DB3=$(awk -F"	" '$2=="db-slave-02"{print $3}' "$SERVERS_TSV")
patch_hba() {
  local host="$1"
...
IP_DB2=$(awk -F"	" '$2=="db-slave-01"{print $3}' "$SERVERS_TSV")
IP_DB3=$(awk -F"	" '$2=="db-slave-02"{print $3}' "$SERVERS_TSV")
patch_hba "$IP_DB1"; patch_hba "$IP_DB2"; patch_hba "$IP_DB3"
