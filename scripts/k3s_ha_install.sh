...
ssh -o StrictHostKeyChecking=no root@"$MASTER_01_IP" "bash -s" <<'EOS'
set -u
set -o pipefail
...
echo "OK : master-01 opérationnel"
...
ssh -o StrictHostKeyChecking=no root@"$MASTER_02_IP" "bash -s" <<EOS
set -u
set -o pipefail
...
echo "OK : master-02 opérationnel"
echo "OK" > "\$ST/STATE"
EOS
...
ssh -o StrictHostKeyChecking=no root@"$MASTER_03_IP" "bash -s" <<EOS
set -u
set -o pipefail
...
echo "OK : master-03 opérationnel"
echo "OK" > "\$ST/STATE"
EOS
