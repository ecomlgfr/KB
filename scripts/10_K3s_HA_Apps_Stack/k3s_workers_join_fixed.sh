...
for WORKER in "${WORKERS[@]}"; do
    ...
    ssh -o StrictHostKeyChecking=no root@"$WORKER_IP" "bash -s" <<EOS
set -u
set -o pipefail
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Installation worker $WORKER"
...
echo "Installation K3s agent (join via \${LB_API}:6443)..."
...
echo "OK : worker $WORKER opérationnel"
echo "OK" > "\$ST/STATE"
EOS
