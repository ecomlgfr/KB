...
echo "╔════════════════════════════════════════════════════════════════════╗"
echo "║    K3S - Déploiement Chatwoot CORRIGÉ                             ║"
...
echo ""
echo "Déploiement Chatwoot - Architecture KeyBuzz (CORRIGÉ)"
...
echo "Suppression des déploiements existants (si présents)..."
kubectl delete deployment chatwoot-web -n chatwoot 2>/dev/null || echo "  (déjà supprimé)"
kubectl delete deployment chatwoot-worker -n chatwoot 2>/dev/null || echo "  (déjà supprimé)"
kubectl delete job chatwoot-db-migrate -n chatwoot 2>/dev/null || echo "  (déjà supprimé)"
...
kubectl apply -f - <<'EOF'
---
# Job de migration DB (COMMANDE CORRIGÉE)
apiVersion: batch/v1
kind: Job
metadata:
...
echo "Attente du job de migration (120s max)..."
kubectl wait --for=condition=complete --timeout=120s job/chatwoot-db-migrate -n chatwoot 2>/dev/null || {
    echo -e "$WARN Migration prend plus de temps, vérification des logs..."
    kubectl logs -n chatwoot job/chatwoot-db-migrate --tail=30
}

echo ""
echo "Attente des pods Web (90s max)..."
kubectl wait --for=condition=ready --timeout=90s pod -l component=web -n chatwoot 2>/dev/null || {
    echo -e "$WARN Pods pas encore prêts"
}

echo ""
echo "Attente des pods Workers (60s max)..."
kubectl wait --for=condition=ready --timeout=60s pod -l component=worker -n chatwoot 2>/dev/null || {
...
echo "  ./13_deploy_superset_FIXED.sh"
echo ""
