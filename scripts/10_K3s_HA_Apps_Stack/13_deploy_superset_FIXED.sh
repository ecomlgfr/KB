...
echo "╔════════════════════════════════════════════════════════════════════╗"
echo "║    K3S - Déploiement Superset CORRIGÉ                             ║"
...
echo ""
echo "Déploiement Superset - Architecture KeyBuzz (CORRIGÉ)"
...
# ═══════════════════════════════════════════════════════════════════
# ÉTAPE 0.5: NETTOYAGE DES RESSOURCES EXISTANTES
# ═══════════════════════════════════════════════════════════════════

echo ""
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║ ÉTAPE 0.5: Nettoyage des ressources existantes                ║"
echo "╚═══════════════════════════════════════════════════════════════╝"
echo ""

echo "Suppression des ressources existantes (si présentes)..."
kubectl delete deployment superset -n superset 2>/dev/null || echo "  (déjà supprimé)"
kubectl delete job superset-init-db -n superset 2>/dev/null || echo "  (déjà supprimé)"
kubectl delete job superset-create-admin -n superset 2>/dev/null || echo "  (déjà supprimé)"

echo "Attente de la suppression complète (10s)..."
sleep 10
...
kubectl apply -f - <<'EOF'
---
# Job d'initialisation DB (AVEC PSYCOPG2)
apiVersion: batch/v1
kind: Job
metadata:
  ...
  template:
    ...
    spec:
      ...
      - name: pip-cache
        emptyDir: {}
---
# Job création admin user (AVEC PSYCOPG2)
apiVersion: batch/v1
kind: Job
metadata:
    ...
    spec:
      ...
      - name: config
        configMap:
          name: superset-config
---
# Deployment Superset (2 réplicas) (AVEC PSYCOPG2)
apiVersion: apps/v1
kind: Deployment
metadata:
  name: superset
...
echo "Attente du job init-db (180s max)..."
kubectl wait --for=condition=complete --timeout=180s job/superset-init-db -n superset 2>/dev/null || {
    echo -e "$WARN Init DB prend plus de temps, vérification des logs..."
    kubectl logs -n superset job/superset-init-db --tail=30
}

echo ""
echo "Attente du job create-admin (120s max)..."
kubectl wait --for=condition=complete --timeout=120s job/superset-create-admin -n superset 2>/dev/null || {
    echo -e "$WARN Create admin prend plus de temps, vérification des logs..."
    kubectl logs -n superset job/superset-create-admin --tail=30
}

echo ""
echo "Attente des pods Superset (120s max)..."
kubectl wait --for=condition=ready --timeout=120s pod -l app=superset -n superset 2>/dev/null || {
